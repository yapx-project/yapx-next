...
export default function SignUpView() {
  ...
  const getName = (e: InputEvent) => {
    setName(e.target.value);
  };
  const getEmail = (e: InputEvent) => {
    setEmail(e.target.value);
  };
  const getLogin = (e: InputEvent) => {
    setLogin(e.target.value);
  };
  const getPassword = (e: InputEvent) => {
    setPassword(e.target.value);
  };
  const getPasswordRep = (e: InputEvent) => {
  ...
  return (
    <div className="flex h-dvh flex-col justify-between bg-primary_gray">
      ...
      <main className="m-5">
        <h1 className="mb-4 font-roboto text-2xl">Регистрация</h1>
      ...
      <footer className="h-14 border-t border-t-gray-300 bg-white"></footer>
    </div>
  );
}
