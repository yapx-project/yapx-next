...
import { findLikedPosts } from "@/entities/users/service";
...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  switch (method) {
    case "GET":
      return getLikedPostsHandler(req, res, id);
    default:
...
export async function getLikedPostsHandler(
  req: NextApiRequest,
  res: NextApiResponse,
  user_id: string,
) {
  ...
  const postData = await findLikedPosts(user_id, limit, offset);
